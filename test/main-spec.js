...
describe('main', () => {
  ...
  parameterized(
    ...
    `<a conv="./">Text</a>`,
    [
      {selector: '*', attrPattern: 'href', attrReplace: 'conv'}
    ]
  );

  parameterized(
    `<foo src="./"></foo><bar src="./"></bar>`,
    `<foo crs="./"></foo><bar src="./"></bar>`,
    [
      {selector: 'foo', attrPattern: 'src', attrReplace: 'crs'}
    ]
  );

  parameterized(
    `<a href="./" (click)="foo()"></a>`,
    `<a href="./" ng-click="foo()"></a>`,
    [
      {selector: '*', attrPattern: '\\((.*)\\)', attrReplace: 'ng-$1'}
    ]
  );
  ...
  parameterized(
    ...
    [
      {
        selector: '*',
        attrPattern: 'selected',
        attrReplace: 'replaced'
      }
    ]
  );
